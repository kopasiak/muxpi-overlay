...
src_compile() {
	...
	set -- env GOPATH="${S}" GOARCH="${GOARCH:-$ARCH}" GOARM="${GOARM:-7}" go build -v -work -x ./...
	echo "$@"
	"$@" || die "compile failed"
	cd man
	emake build
...
src_install() {
	pushd "src/${EGO_PN}" > /dev/null || die
	set -- env GOPATH="${S}" GOARCH="${GOARCH:-$ARCH}" GOARM="${GOARM:-7}" go install -v -work -x ./...
	echo "$@"
	"$@" || die
	dobin "${S}"/bin/linux_arm/influx*
